...
pub enum AssemblerError {
    ...
    LexerError { message: String, loc: SourceLocation },
    /// Error during the parsing phase.
    ParserError { message: String, loc: SourceLocation },
    /// Error related to symbols (labels).
    SymbolError { message: String, loc: SourceLocation }, // Loc might be where the symbol was used
    /// Error during instruction encoding.
    EncodingError { message: String, loc: SourceLocation }, // Loc might be the instruction location
...
impl fmt::Display for AssemblerError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            AssemblerError::LexerError { message, loc } => write!(f, "Lexer Error at {}: {}", loc, message),
            AssemblerError::ParserError { message, loc } => write!(f, "Parser Error at {}: {}", loc, message),
            AssemblerError::SymbolError { message, loc } => write!(f, "Symbol Error near {}: {}", loc, message),
            AssemblerError::EncodingError { message, loc } => write!(f, "Encoding Error near {}: {}", loc, message),
...
pub fn err_lex(message: impl Into<String>, line: usize, col: usize) -> AssemblerError {
    AssemblerError::LexerError { message: message.into(), loc: SourceLocation { line, col } }
}

pub fn err_parse(message: impl Into<String>, line: usize, col: usize) -> AssemblerError {
    AssemblerError::ParserError { message: message.into(), loc: SourceLocation { line, col } }
}

pub fn err_symbol(message: impl Into<String>, line: usize, col: usize) -> AssemblerError {
    AssemblerError::SymbolError { message: message.into(), loc: SourceLocation { line, col } }
}

pub fn err_encoding(message: impl Into<String>, line: usize, col: usize) -> AssemblerError {
    AssemblerError::EncodingError { message: message.into(), loc: SourceLocation { line, col } }
}
